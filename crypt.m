...
while(stay)
    switch(whichCipher)
        ...
        case 'vig'
            charVec = vig(word,key,method);
            stay = 0;
            
        otherwise
            whichCipher = input('Please input a valid cipher. (''porta'', ''beaufort'', ''auto'', ''vig'' or ''run'': ','s');
    end
end
...
%Vignere Cipher
    function Output = vig(VigPhrase,key,method)
        ...
        VigPhrase = upper(VigPhrase);
        VigPhrase = regexprep(VigPhrase,'[ ~!@#$%^&*()_\+\-\=`1234567890{}\[\]\\|:;"''<,>.?\/]','');
        key = upper(key);
        key = regexprep(key,'[ ~!@#$%^&*()_\+\-\=`1234567890{}\[\]\\|:;"''<,>.?\/]','');
        newKey = '';
        for i = 1:length(VigPhrase)
            newKey(i) = key(mod(i-1,length(key))+1);
        end
        Output = '';
        ...
        switch(method)
            case 'd'
                for i = 1:length(VigPhrase)
                    for j = 1:26
                        if(B(double(newKey(i))-64,j) == VigPhrase(i))
                            Output(i) = B(1,j);
                            break;
                        end
                    end
            ...
            otherwise
                for i = 1:length(newKey)
                    Output(i) = B(double(VigPhrase(i))-64,double(newKey(i))-64);
                end
        end
    end
