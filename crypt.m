function charArray = crypt(word,key,whichCipher,method,spaces)
...
while(stay)
    switch(whichCipher)
        ...
        case 'auto'
            charVec = AutoKeyCipher(word,key,method);
            stay = 0;
            
        case 'run'
            charVec = runningkey(word,key,method);
            stay = 0;
            
        otherwise
            whichCipher = input('Please input a valid cipher. (''porta'', ''beaufort'', ''auto'', or ''run'': ','s');
    end
end
...
%Auto Key Cipher
    function encrypted = AutoKeyCipher(HideText,KeyWord,method)
        ...
        switch(method)
            case 'd'
                KeyStream = upper(KeyWord);
                for n = 1:length(HideText)
                    for h = 1:26
                        if Tabula(double(KeyStream(n)) - 64,h) == HideText(n)
                            encrypted(n) = Tabula(1,h);
                            KeyStream(length(KeyStream) + 1) = encrypted(n);
                            break;
                        end
                    end
                end
            otherwise
                KeyStream = upper(strcat(KeyWord, HideText));
                for n = 1:length(HideText)
                    encrypted(n) = Tabula(double(KeyStream(n)) - 64, double(HideText(n)) - 64);
                end
        end
    end

%Running Key cipher
    function output=runningkey(message,keyword,process)


        rkc=['ABCDEFGHIJKLMNOPQRSTUVWXYZ';...
             'BCDEFGHIJKLMNOPQRSTUVWXYZA';...
             'CDEFGHIJKLMNOPQRSTUVWXYZAB';...
             'DEFGHIJKLMNOPQRSTUVWXYZABC';...
             'EFGHIJKLMNOPQRSTUVWXYZABCD';...
        ...
        switch (process)
            case 'd'
                for i = 1:length(message)
                   for j = 1:26
                      if(rkc(double(keyword(i)) - 64,j) == message(i))
                         output(i) = rkc(1,j); 
                         break;
                      end
                   end
            ...
            otherwise
                for i=1:length(message)
                    output(i)=rkc(double(keyword(i))-64,double(message(i))-64);
                end
        end
    end
